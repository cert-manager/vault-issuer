...
type LeaderElectionConfig struct {
	...
	// ensure no more than one instance of vault-issuer operates at a time
	Enabled bool
...
// Options is a struct to hold options for vault-issuer
type Options struct {
...
func New() *Options {
	return &Options{
		IssuerOptions: controller.NewDefaultOptions(),
	}
}
...
// Validate checks the flag values for contradictions that would otherwise
// surface as confusing errors at reconcile time. Must run after Complete.
func (o *Options) Validate() error {
	var errs []error

	errs = append(errs, o.IssuerOptions.Validate())

	if o.LeaderElectionConfig.Enabled {
		if o.LeaderElectionConfig.LeaseDuration <= 0 || o.LeaderElectionConfig.RenewDeadline <= 0 {
			errs = append(errs, errors.New("--leader-election-lease-duration and --leader-election-renew-deadline must be positive"))
		}
		if o.LeaderElectionConfig.RenewDeadline >= o.LeaderElectionConfig.LeaseDuration {
			errs = append(errs, fmt.Errorf(
				"--leader-election-renew-deadline (%s) must be less than --leader-election-lease-duration (%s)",
				o.LeaderElectionConfig.RenewDeadline, o.LeaderElectionConfig.LeaseDuration))
		}
	}

	if o.ReadyzPort < 1 || o.ReadyzPort > 65535 {
		errs = append(errs, fmt.Errorf("--readiness-probe-port must be a valid port, got %d", o.ReadyzPort))
	}
	if o.MetricsPort < 1 || o.MetricsPort > 65535 {
		errs = append(errs, fmt.Errorf("--metrics-port must be a valid port, got %d", o.MetricsPort))
	}
	if o.MetricsPort == o.ReadyzPort {
		errs = append(errs, fmt.Errorf("--metrics-port and --readiness-probe-port must differ, both are %d", o.MetricsPort))
	}

	if o.log.level < 1 || o.log.level > 5 {
		errs = append(errs, fmt.Errorf("--log-level must be between 1 and 5, got %d", o.log.level))
	}
...
func (o *Options) addAppFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVar(&o.LeaderElectionConfig.Enabled, "leader-elect", true, ""+
		"If true, vault-issuer will perform leader election between instances to ensure no more "+
		"than one instance of vault-issuer operates at a time")
